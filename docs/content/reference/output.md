...
describes what users get. Pick a format with `-o`, or let danviet choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
danviet <command> -o table   # aligned columns for reading
danviet <command> -o jsonl   # one JSON object per line, for piping
danviet <command> -o json    # a single JSON array
danviet <command> -o csv     # spreadsheet friendly
danviet <command> -o tsv     # tab-separated
danviet <command> -o url     # just the URL column
danviet <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
danviet <command> --fields id,title,url
```
...
```bash
danviet <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
danviet <command>            # a table, because this is a terminal
danviet <command> | wc -l    # JSONL, because this is a pipe
```
